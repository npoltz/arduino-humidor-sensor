...
#define SLEEP_TIME_SECONDS 600 // 10 minutes
...
void setup() {
  ...
  if (wifiClient.connect(hostIp, hostPort)) {
    ...
    int httpResponseCode = httpClient.POST("{\"sensorId\":\"" + String(sensorId) + "\",\"temperature\":\"" + String(temperature) + "\",\"relativeHumidity\":\"" + String(humidity) + "\"}");
